...
bool NodeManager::initNode(Node &node)
{
    ...
    */
    ...
    else                              //文件不为空
    {
        ...
        //battleSystem.initbattle(node); // 初始化战斗数据
        //rewardSystem.initreward(node); // 初始化奖励数据
        //storySystem.initstory(node); // 初始化剧情数据
    }
    file.close();
    return true;
}
// 根据ID查找节点
const Node& NodeManager::NodeManager_GetNodeById(const Node& node, int id) {
...
// 获取某类型的所有节点
std::vector<Node>  NodeManager::NodeManager_GetNodesByType(const Node& node, NodeType type) {
...
// 节点是否存在
bool  NodeManager::NodeManager_NodeExists(const Node& node, int id) {
    // TODO: 遍历nodes查找匹配ID的节点
        if (node.id == id) {
            return true;
        }
