...
#ifdef _WIN32
#include "../WindowsCompatible/dirent.h"        // 用于目录操作    
#include <io.h>         // 用于代替 unlink
#include <process.h>    // 用于代替 unlink
#else
#define _DEFAULT_SOURCE // 确保定义了POSIX宏以启用dirent.h功能
#define _BSD_SOURCE    // 兼容旧版本的POSIX功能
#define _XOPEN_SOURCE 500 // 确保启用POSIX.1-2001功能
#include <unistd.h>    // 用于 unlink 删除文件
#include <sys/types.h> // 添加sys/types.h以确保dirent.h的依赖被正确包含
#include <dirent.h>
#include <cstring> // 添加cstring以确保str相关函数可用
#endif
...
bool SaveManager::saveGame(const std::string &filePath, const PlayerState &player, int currentNodeId) {
	...
	try {
		...
		std::stringstream ss;
		ss << std::put_time(std::localtime(&in_time_t), "%Y-%m-%d %H:%M:%S");		// C4996 警告，localtime() 可能导致内存越界漏洞
...
bool SaveManager::deleteSave(const std::string &filePath) {
	try {
		if (saveExists(filePath)) {
			if (unlink(filePath.c_str()) == 0) {  // 使用 POSIX API		// C4996 警告，unlink() 可能导致内存越界漏洞
